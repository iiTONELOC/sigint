...
- **Multi-pane layout** — Resizable split panes with globe and data table side by side. Minimize/restore with position memory, drag-to-move (insert beside target, not swap), horizontal/vertical toggle, named layout presets (save/load/update/delete). Layout persists to IndexedDB. Mobile responsive — single pane with tab switching under 768px.
...
- **Correlation engine** — Cross-source intelligence derived from all live data. Grid-indexed O(n) spatial matching correlates GDELT conflicts with nearby FIRMS fires, earthquakes with secondary fires, severe weather with vessel density, and military aircraft with conflict zones. Anomaly detection via 7-day rolling per-country baselines (persisted to IndexedDB) flags activity spikes relative to historical norms. News linking matches RSS articles to active regions. Output: prioritized intel products for the Intel Feed and context-scored alerts (1-10) with factor breakdowns for the Alert Log.
- **Watch mode** — Automated globe tour cycling through alerts, intel products, or both interleaved by priority. Controlled from globe overlay (ALERTS/INTEL/ALL dropdown). 8-second dwell per item with progress bar. ISS-level reveal (gentle rotate, no deep zoom). Dossier + alert + intel panes auto-opened on watch start. Pause on manual interaction, resume from where you left off. Watch layout auto-repairs closed panes within 3 seconds.
- **Military aircraft classification** — Heuristic identification using ICAO type codes (50+ military designators), operator keywords (15 military strings), and US DoD ICAO24 hex range (AE0000–AFFFFF). ~15,700 aircraft flagged. Orange-red rendering, MIL badge in ticker/dossier, military filter (ALL/MIL/CIV) in aircraft controls, correlation engine cross-references with conflict zones.
...
The application uses a shared data context (`DataContext`) with a multi-pane layout managed by `PaneManager`. All state — data hooks, selection, filters, view controls, correlation engine, watch mode — lives in the context, accessible to any component via `useData()`. A correlation engine (`lib/correlationEngine.ts`) runs as a `useMemo` on every data refresh, deriving intelligence products and context-scored alerts from cross-source spatial matching, anomaly detection, and news linking. `App.tsx` wraps everything in `DataProvider`, and `AppShell` renders the app-level chrome (Header, Ticker) around the pane area.
