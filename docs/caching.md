...
services and DataWorker source owners use this cross-runtime contract.

Each enum value is one exact IndexedDB object-store key. Settings-only labels
and groups stay in `src/client/settings/model/cache.ts`.
...
Change `CacheKey` when a stored data shape needs a new key. Do not create a
cache-key string in a feature module.
