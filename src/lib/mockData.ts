export interface DataPoint {
  id: string;
  type: "ships" | "aircraft" | "events" | "quakes";
  lat: number;
  lon: number;
  // Ships
  name?: string;
...
export function generateMockData(): DataPoint[] {
  const ships: DataPoint[] = Array.from({ length: 55 }, (_, i) => {
    ...
    const types = ["Cargo", "Tanker", "Container", "Bulk Carrier", "Fishing", "Tug"];
    ...
    return {
      ...
      flag: flags[i % 9],
      speed: rnd(2, 20).toFixed(1),
      heading: Math.floor(rnd(0, 360)),
    };
  });

  const aircraft: DataPoint[] = Array.from({ length: 40 }, (_, i) => {
    let lat = rnd(-50, 65),
      lon = rnd(-170, 170);
    if (i < 12) {
      lat = rnd(38, 55);
  ...
  const events: DataPoint[] = Array.from({ length: 35 }, (_, i) => {
    ...
    const cats = ["Protest", "Military Action", "Diplomatic", "Conflict", "Humanitarian", "Political Crisis"];
    const heads = [
  ...
  const quakes: DataPoint[] = Array.from({ length: 22 }, (_, i) => {
    ...
    return {
      ...
      lat,
      lon,
      magnitude: rnd(1.2, 7.1).toFixed(1),
  ...
  return [...ships, ...aircraft, ...events, ...quakes];
}
