...
type FeedFilter = "all" | "events" | "quakes" | "fires";
...
export function IntelFeedPane() {
  ...
  // ── Filter to intel types only ──────────────────────────────────

  const feedTypes = useMemo(() => new Set(["events", "quakes", "fires"]), []);
  ...
  const typeCounts = useMemo(() => {
    const counts: Record<string, number> = { events: 0, quakes: 0, fires: 0 };
  ...
  return (
    <div className="w-full h-full flex flex-col bg-sig-bg overflow-hidden">
      ...
      <div className="shrink-0 flex items-center gap-1 px-2 py-1 border-b border-sig-border/40">
        ...
        {(["events", "quakes", "fires"] as const).map((type) => {
        ...
        {feedItems.map((item) => {
          ...
          return (
            <div
              ...
              className={`px-3 py-2 border-b border-sig-border/20 cursor-pointer transition-colors ${
                isSelected
                  ? "bg-sig-accent/10"
                  : "bg-transparent hover:bg-sig-panel/40"
              }`}
            >
              {/* Row 1: Icon + headline + age */}
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sig-text text-(length:--sig-text-md) mt-0.5 line-clamp-2 leading-snug">
                    {headline}
                  </div>
                </div>
              </div>

              {/* Row 2: Source + location + actions */}
